...
/// [ApiWatchlistMovie] merupakan kode class untuk menarik data dari link api.
class ApiWatchlistMovie {
  final String apikey = '98c580af951d9cbf543ece8df822519c';

  Future<List<WatchlistMovie>?> getwatchlist(int id) async {
    try {
      final response = await http.get(
        Uri.parse(
            'https://api.themoviedb.org/3/account/$id/watchlist/movies?api_key=$apikey'),
        headers: {
          'Authorization':
      ...
      if (response.statusCode == 200) {
        ...
        var getwatchlistmovie = jsonData['results'] as List;
        var listwatchlistmovie =
            getwatchlistmovie.map((i) => WatchlistMovie.fromJson(i)).toList();
        return listwatchlistmovie;
      } else {
